...
func Load() (*Config, error) {
	...
	if jwtAccessTokenSecret == "" {
		return nil, fmt.Errorf("JWT_ACCESS_TOKEN_SECRET environment variable must be set")
	}
	if jwtRefreshTokenSecret == "" {
		return nil, fmt.Errorf("JWT_REFRESH_TOKEN_SECRET environment variable must be set")
	}
	...
	config := &Config{
		Database: DatabaseConfig{
			...
			User:     getEnv("DB_USER", "gocrm"),
			Password: getEnv("DB_PASSWORD", ""),
			SSLMode:  getEnv("DB_SSL_MODE", "disable"),
			DbString: getEnv("GOOSE_DBSTRING", "host=localhost port=5432 user=gocrm password=hadeed@0987 dbname=gocrm sslmode=disable"),
		},
		Server: ServerConfig{
		...
		JWT: JWTConfig{
			Secret:                     jwtSecret,
			ACCESS_TOKEN_SECRET:        jwtAccessTokenSecret,
			REFRESH_TOKEN_SECRET:       jwtRefreshTokenSecret,
			ExpiryHours:                getEnvAsInt("JWT_EXPIRY_HOURS", 24),
			ACCESS_TOKEN_EXPIRY_HOURS:  getEnvAsInt("JWT_ACCESS_TOKEN_EXPIRY_HOURS", 10),
			REFRESH_TOKEN_EXPIRY_HOURS: getEnvAsInt("JWT_REFRESH_TOKEN_EXPIRY_HOURS", 24),
